...
def convert_income_label(y_label):
    y_ = y_label.squeeze()
    # print(y_)
    y_.apply_(lambda x: 1 if x >=4 else 0)
    # print(y_)
    ...
    for i_ in range(iter_max):
        ...
        with tqdm(dataloader) as pbar:
            for X, Y in pbar:
                ...
                y_out= clf(X)
                y_labels = convert_income_label(Y-1)
...
run_raw_classification(dataloader_dict['train'], lr=2e-4, iter_max=500)
