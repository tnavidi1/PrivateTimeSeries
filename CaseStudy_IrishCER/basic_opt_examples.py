...
def check_basic_csc(param_set=None, plotfig=False, debug=False):
    ...
    ##################################
    if debug:
        _check_verbose_sol_byDIFFCP(x_sol, T)
...
def run_battery(dataloader, params=None):
    ...
    with tqdm(dataloader) as pbar:
        for k, (D, Y) in enumerate(pbar):
            ...
            construct_QP_battery_w_D_cvx(param_set=params, d=D[0], p=None, plotfig=True)
            construct_QP_battery_w_D_conic(param_set=params, d=D[0], p=None, plotfig=True)
            if k > 2:
